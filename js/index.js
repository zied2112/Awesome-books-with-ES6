...
const dateAndTime = () => {
  setInterval(() => {
    const now = DateTime.now().toFormat('dd LLL yyyy hh:mm:ss a');
    timeAndDate.innerHTML = now;
  }, 1000);
};
...
class AwesomeBook {
  ...
  displayBook() {
    ...
    <div class="display__books__header">
    <h1>Book Store</h1>
    </div>
    ${this.bookArray
    .map(
...
listBtn.addEventListener('click', () => {
  displayContentContainer.style.display = 'flex';
...
contactBtn.addEventListener('click', () => {
  ...
  contactInfo.innerHTML = contentData.getcontent();
  displayContentContainer.style.display = 'none';
  form.style.display = 'none';
